...
-- Adds signed payload metadata to public.audit_event, computes server-side
-- payload/chain hashes for every future insert, and exposes a controlled
-- append RPC so browser clients do not need direct INSERT policies.
...
UPDATE public.audit_event
SET chain_hash = encode(digest(COALESCE(payload_hash, '') || id::text, 'sha256'), 'hex')
...
CREATE INDEX IF NOT EXISTS idx_audit_event_accession_at
  ON public.audit_event (tenant_id, accession_id, at DESC)
  WHERE accession_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_audit_event_hash
  ON public.audit_event (tenant_id, payload_hash);
...
BEGIN
  ...
  NEW.payload := COALESCE(
    ...
    jsonb_build_object(
      ...
      'accessionId', NEW.accession_id,
      'field', NEW.field,
      'oldValue', NEW.old_value,
      'newValue', NEW.new_value,
      'reason', NEW.reason,
      'actorLabel', NEW.actor_label,
  ...
  NEW.chain_hash := encode(
    digest(COALESCE(v_previous_chain_hash, '') || NEW.payload_hash || NEW.id::text, 'sha256'),
    'hex'
  );
  RETURN NEW;
END;
...
CREATE TRIGGER trg_prepare_audit_event_append
...
CREATE OR REPLACE FUNCTION public.append_audit_event(
  ...
  p_accession_id TEXT DEFAULT NULL,
  p_field TEXT DEFAULT NULL,
  p_old_value JSONB DEFAULT NULL,
  p_new_value JSONB DEFAULT NULL,
  p_reason TEXT DEFAULT NULL,
  p_actor_label TEXT DEFAULT NULL,
  p_source_module TEXT DEFAULT 'lims',
...
BEGIN
  ...
  IF p_entity IS NULL OR length(trim(p_entity)) = 0 THEN
    RAISE EXCEPTION 'audit entity is required';
  END IF;
