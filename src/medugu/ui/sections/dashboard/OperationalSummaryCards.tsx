...
export function OperationalSummaryCards({ summary }: { summary: OperationalDashboardSummary }) {
  const cards = [
    ...
    {
      label: "Critical / urgent cases",
      value: summary.criticalUrgentCases,
      tone: "text-destructive",
    },
    {
      label: "Critical/high queue items",
    ...
    <div className="grid gap-2 md:grid-cols-4">
      {cards.map((card) => (
        <div key={card.label} className="rounded border border-border bg-card p-3">
          ...
          <div className={`mt-1 text-xl font-semibold ${card.tone ?? "text-foreground"}`}>
            {card.value}
          </div>
        </div>
      ))}
