...
assert(
  cre.decisions.some((decision) => decision.ruleCode === "CRE_ALERT"),
  "CRE sterile-site case must trigger CRE_ALERT.",
);
assert(
  cre.decisions.some(
    (decision) => decision.ruleCode === "CRE_ALERT" && decision.timing === "immediate",
  ),
...
assert(
  cre.decisions.some(
    (decision) =>
      decision.ruleCode === "CRE_ALERT" && decision.message.toLowerCase().includes("carbapenem"),
  ),
...
assert(
  cre.decisions.some(
    (decision) =>
      decision.ruleCode === "CRE_ALERT" && decision.actions.includes("contact_plus_precautions"),
  ),
...
assert(
  mrsa.decisions.some((decision) => decision.ruleCode === "MRSA_ALERT"),
  "MRSA bloodstream case must trigger MRSA_ALERT.",
);
assert(
  mrsa.decisions.some(
    (decision) => decision.ruleCode === "MRSA_ALERT" && decision.notify.length > 0,
  ),
...
assert(
  vre.decisions.some((decision) => decision.ruleCode === "VRE_ALERT"),
  "VRE case must trigger VRE_ALERT.",
);

const cauris = evaluateIPC(ipcAcceptanceScenarioCases.candidaAurisScreenPositiveCase);
assert(
  cauris.decisions.some((decision) => decision.ruleCode === "CAURIS_ALERT"),
  "Candida auris case must trigger CAURIS_ALERT.",
);
assert(
  cauris.decisions.some(
    (decision) => decision.ruleCode === "CAURIS_ALERT" && decision.timing === "immediate",
  ),
