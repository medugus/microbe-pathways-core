...
assert(
  !!pendingItem,
  "Pending restricted AMS item should appear in dashboard queue as ams_pending_approval.",
);
...
const noActionAmsItems = dashboard.items.filter(
  (item) =>
    item.accessionId === amsAcceptanceScenarioCases.noAmsActionCase.id &&
    item.sourceModule === "AMS",
);
assert(
  noActionAmsItems.length === 0,
  "No-AMS-action case should not create a false AMS dashboard queue item.",
);
...
const releaseRelevantAmsItems = releaseRelevantDashboard.items.filter(
  (item) => item.sourceModule === "AMS",
);
assert(
...
const enteredRestrictedDashboard = deriveOperationalDashboard(
  toAccessionsMap([enteredRestrictedCase]),
);
assert(
  enteredRestrictedDashboard.items.some(
    (item) => item.category === "ams_restricted" || item.category === "ams_pending_approval",
  ),
  "Entered restricted AST result should appear in dashboard AMS pending/restricted queue.",
);

const approvedRestrictedCase = clone(enteredRestrictedCase);
approvedRestrictedCase.id = "AMS-REL-003";
approvedRestrictedCase.accessionNumber = "MB26-AMJHEA-MEM-APPROVED";
approvedRestrictedCase.amsApprovals = [
  {
    id: "ams_req_mem_approved",
    astId: approvedRestrictedCase.ast.find((row) => row.antibioticCode === "MEM")!.id,
    isolateId: "iso_rel_1",
    antibioticCode: "MEM",
    status: "approved",
    dueBy: "2026-04-26T12:00:00.000Z",
    requested: { at: "2026-04-25T11:00:00.000Z", actor: "AMS pharmacist" },
    decided: { at: "2026-04-25T11:15:00.000Z", actor: "AMS pharmacist" },
  },
];

const approvedRestrictedContext = deriveAMSReleaseContext(approvedRestrictedCase);
assert(
  approvedRestrictedContext.pendingApprovalCount === 0,
  "Approved restricted AST result should clear pending AMS approval count.",
);
assert(
...
assert(
  !approvedRestrictedDashboard.items.some((item) => item.category === "ams_pending_approval"),
  "Approved restricted AST result should not appear in dashboard pending AMS queue.",
);
