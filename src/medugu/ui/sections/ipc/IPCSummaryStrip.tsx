...
export function IPCSummaryStrip({ decisions, openActionCount, episodeCounts, localWatchSummary }: IPCSummaryStripProps) {
  ...
  return (
    <div className="rounded-md border border-border bg-muted/20 p-3">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <span className="rounded bg-background px-2 py-1 font-medium text-foreground">{summaryText}</span>
        <span className="rounded bg-background px-2 py-1 text-muted-foreground">highest severity: {highestSeverity}</span>
        <span className="rounded bg-background px-2 py-1 text-muted-foreground">
          episodes — new: {episodeCounts.newCount}, repeat: {episodeCounts.repeatCount}, clearance: {episodeCounts.clearanceCount}
        </span>
        {localWatchSummary && (
          <span className="rounded bg-background px-2 py-1 text-muted-foreground">{localWatchSummary}</span>
        )}
      </div>
    </div>
  );
