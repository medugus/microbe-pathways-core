...
export function IPCSignalCard({
  ...
  return (
    <article
      className={`space-y-2 rounded-md border border-border bg-card p-3 ${CARD_TONE[severity]}`}
    >
      ...
      <p className="text-xs text-muted-foreground">
        Organism / phenotype: {organismLabel}
        {decision.phenotypes.length ? ` · ${decision.phenotypes.join(", ")}` : ""}
      </p>
      <p className="text-xs text-muted-foreground">Specimen context: {specimenContext}</p>
      <p className="text-xs text-muted-foreground">
        Ward/location context: {ward || "not available"}
      </p>
      <p className="text-xs text-muted-foreground">Trigger reason: {triggerReason}</p>
      <p className="text-xs text-muted-foreground">
        Required action summary:{" "}
        {decision.actions.map((a) => a.replaceAll("_", " ")).join(", ") || "not available"}
      </p>
      <p className="text-xs text-muted-foreground">
        Notification targets: {decision.notify.join(", ") || "not available"}
      </p>
      <p className="text-xs text-muted-foreground">
        Escalation timing: {decision.timing.replaceAll("_", " ")}
      </p>
      {signal && (
        <p className="text-xs text-muted-foreground">
          Generated by rule:{" "}
      ...
      {decision.clearanceProgress && (
        <p className="text-xs text-muted-foreground">
          Clearance case context: {decision.clearanceProgress.negativeCount}/
          {decision.clearanceProgress.required} negative screens
        </p>
      )}
      {decision.priorAccessionIds && decision.priorAccessionIds.length > 0 && (
        <p className="text-xs text-muted-foreground">
          Prior episode accessions: {decision.priorAccessionIds.join(", ")}
        </p>
      )}
