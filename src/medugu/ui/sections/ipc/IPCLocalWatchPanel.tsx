  ...
  return (
    <section className="space-y-2 rounded-md border border-dashed border-border bg-muted/20 p-3">
      ...
      <div className="space-y-1 text-xs text-muted-foreground">
        ...
        {priorAccessions.length > 0 && (
          <p className="break-all">{priorAccessions.join(", ")}</p>
        )}
        <p>Repeat-patient adjusted count: {repeatAdjustedCount ?? comparableSignals.reduce((sum, s) => sum + s.count, 0)}</p>
      </div>
        ...
        <div className="space-y-1 text-xs">
          ...
          <ul className="space-y-1 text-muted-foreground">
            {comparableSignals.map((s) => (
              <li key={s.ruleCode}>{s.ruleCode}: {s.count}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {wardGrouping.length > 0 && (
        <div className="space-y-1 text-xs">
          ...
          <ul className="space-y-1 text-muted-foreground">
            {wardGrouping.map((w) => (
              <li key={w.ward}>{w.ward}: {w.count}</li>
            ))}
          </ul>
        </div>
      )}
