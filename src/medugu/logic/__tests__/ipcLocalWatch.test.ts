...
const clusterReport = deriveLocalOutbreakWatch(
  ipcAcceptanceScenarioCases.creClusterIcuCases[0],
  clusterMap,
  7,
);
assert(
  clusterReport.summary === "outbreak watch",
  "Three ICU cases in 7 days should produce outbreak watch summary.",
);
assert(
  clusterReport.signalItems.some((item) => item.patientAdjustedCount === 3),
  "Cluster should show patient-adjusted count of 3.",
);

const repeatedMap = toAccessionsMap(ipcAcceptanceScenarioCases.repeatedSamePatientCreCases);
const repeatedReport = deriveLocalOutbreakWatch(
  ipcAcceptanceScenarioCases.repeatedSamePatientCreCases[0],
  repeatedMap,
  7,
);
assert(
...
const noneReport = deriveLocalOutbreakWatch(
  ipcAcceptanceScenarioCases.negativeNoSignalCase,
  noneMap,
  7,
);
assert(
  noneReport.summary === "no local cluster",
  "No comparable IPC cases should yield no local cluster.",
);

const summaryWords = `${clusterReport.summary} ${repeatedReport.summary} ${noneReport.summary}`;
assert(
  !summaryWords.includes("confirmed outbreak"),
  "Watch output must not claim confirmed outbreak.",
);
