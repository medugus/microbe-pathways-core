...
import { derivePatientMicrobiologyHistory, type PriorHistoryRow } from "../../logic/microHistoryEngine";
...
function RelevanceBadges({ row }: { row: PriorHistoryRow }) {
  ...
  if (row.repeatOrganism) {
    items.push("Repeat organism detected in prior accession. Review clinical context before interpreting as recurrence.");
  }
  ...
  if (row.astComparisons.some((c) => c.worsening)) {
    items.push("Current AST shows reduced susceptibility compared with prior result for one or more agents.");
  }
  if (row.sameSpecimenSourceRecurrence) {
    items.push("Same specimen source noted across prior and current accessions.");
  }

  if (items.length === 0) return <span className="text-muted-foreground">No direct comparator signal.</span>;
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.accessionId} className="border-t border-border align-top">
                  ...
                  <td className="px-2 py-2">{row.context === "diagnostic" ? "Diagnostic culture" : "Colonisation/screening"}</td>
                  <td className="px-2 py-2">
                    {row.organisms.length === 0 ? "—" : row.organisms.join(", ")}
                  </td>
                  <td className="px-2 py-2">{row.priorPhenotypes.length === 0 ? "—" : row.priorPhenotypes.join(", ")}</td>
                  <td className="px-2 py-2"><RelevanceBadges row={row} /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
export function MicroHistoryPanel() {
  ...
  const hasAnyRows = history.diagnosticHistory.length + history.colonisationScreeningHistory.length > 0;
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border bg-card p-5">
      ...
      {!hasAnyRows ? (
        <p className="text-sm text-muted-foreground">No prior microbiology accessions for this patient.</p>
      ) : (
        <>
          <HistoryTable title="Prior diagnostic cultures" rows={history.diagnosticHistory} />
          <HistoryTable title="Prior colonisation/screening" rows={history.colonisationScreeningHistory} />
        </>
      )}
      ...
      <section className="space-y-2">
        ...
        <ul className="space-y-1 text-xs">
          {history.currentRelevance.currentMatchesPriorColonisingOrganism && (
            <li>• Current organism matches a prior colonising organism. Review clinical and IPC context.</li>
          )}
          {history.currentRelevance.repeatAlertOrganism && (
            <li>• Repeat organism detected in prior accession. Review clinical context before interpreting as recurrence.</li>
          )}
          {history.currentRelevance.priorColonisationWithRelatedHighRiskGroup && (
            <li>• Prior colonisation with related high-risk organism/group is present. Review clinical and IPC context.</li>
          )}
          {history.currentRelevance.susceptibilityWorsening && (
            <li>• Current AST shows reduced susceptibility compared with prior result for one or more agents.</li>
          )}
          {history.currentRelevance.newResistantPhenotypeFlag && (
            <li>• New resistant phenotype signal appears in current accession compared with available prior history.</li>
          )}
          {!history.currentRelevance.currentMatchesPriorColonisingOrganism &&
            !history.currentRelevance.repeatAlertOrganism &&
            !history.currentRelevance.priorColonisationWithRelatedHighRiskGroup &&
            !history.currentRelevance.susceptibilityWorsening &&
            !history.currentRelevance.newResistantPhenotypeFlag && (
              <li className="text-muted-foreground">No additional prior-history relevance signals detected.</li>
            )}
        </ul>
      </section>
    </div>
